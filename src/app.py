...
def load_data(path):
    df=pd.read_csv(path)
    return df
mpg_df_raw=load_data(path=r'C:\Users\jpscw\Documents\GitLab\StreamlilFirstApp\data\raw\mpg.csv')
mpg_df= deepcopy(mpg_df_raw) #for security



#left_column, middle_column=st.columns([2,1]) #The first column is taking 2 spaces and the second one just opne
